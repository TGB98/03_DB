...
------------------------
-- 한수현
-- 국어국문학과 학과코드 = DEPARTMENT_NO : 001
SELECT * FROM TB_STUDENT;

SELECT STUDENT_NAME FROM TB_STUDENT tbs
JOIN TB_DEPARTMENT tbd ON (tbs.DEPARTMENT_NO = tbd.DEPARTMENT_NO)
WHERE tbd.DEPARTMENT_NO = 001
AND SUBSTR(STUDENT_SSN, 8, 1) = '2'
AND ABSENCE_YN = 'Y';

-- 4. 도서관에서 대출 도서 장기 연체자들을 찾아 이름을 게시하고자 한다.
-- 그 대상자들의 학번이 다음과 같을 때 대상자들을 찾는 적절한 SQL문 작성.
SELECT * FROM TB_STUDENT;
...
-- 6. 춘 기술대학교는 총장을 제외하고 모든 교수들이 소속 학과를 갖고 있다.
-- 그럼 춘 기술대학교 총장의 이름을 알아낼 수 있는 SQL 문장을 작성.
SELECT PROFESSOR_NAME FROM TB_PROFESSOR
WHERE DEPARTMENT_NO IS NULL;

-- 7. 전산상의 착오로 학과가 지정되어있지 않은 학생이 있는지 확인하고자 한다.
-- 어떠한 SQL 문장을 사용하면 될 것인지 작성.
SELECT * FROM TB_STUDENT;
SELECT STUDENT_NAME FROM TB_STUDENT
WHERE DEPARTMENT_NO IS NULL;
...
-- 재학중인 학생들의 학번, 이름, 주민번호를 출력하는 구문 작성.
SELECT * FROM TB_STUDENT;
SELECT STUDENT_NO, STUDENT_NAME, STUDENT_SSN
FROM TB_STUDENT
WHERE (STUDENT_NO, STUDENT_ADDRESS) IN (
