...
-- 문자열을 앞에서부터 검색해 첫번째 B의 위치 조회.
-- AABAACAABBAA
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 3

-- 문자열을 5번째 문자부터 검색해 첫번째 B의 위치 조회.
SELECT INSTR('AABAACAABBAA', 'B' , 5) FROM DUAL; -- 9

-- 문자열을 5번째 문자부터 검색해 두번째 B의 위치 조회.
...
SELECT ROUND(123.456) FROM DUAL; -- 123, 소수점 첫 번째 자리에서 반올림(기본값).

SELECT ROUND(123.456, 1) FROM DUAL; -- 123.5 , 소수점 두 번째 자리에서 반올림(소수점 첫번째 자리까지 표기). 
...
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 2025.02.08
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL;  -- 2024.09.08

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함.
SELECT LAST_DAY(SYSDATE) FROM DUAL; -- 2024.10.31
SELECT LAST_DAY('2020-02-01') FROM DUAL; -- 2020.02.29 

-- EXTRACT : 년, 월, 일 정보를 추출해 리턴(반환).
-- EXTRACT(YEAR FROM 날짜) : 년도만 추출.
-- EXTRACT(MONTH FROM 날짜) : 월만 추출.
-- EXTRACT(DAY FROM 날짜) : 일만 추출.

-- EMPLOYEE 테이블에서 각 사원의
-- 이름, 입사일 조회(입사년도, 월, 일)
-- 2000년 10월 10일
SELECT EMP_NAME,
...
-- 0 : 숫자 한 칸을 의미, 여러 개 작성 시 오른쪽 정렬, 빈 칸이 있으면 0 추가.
...
-- R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우 이전 세기 (1900년대)
-- 			절반(50년) 미만인 경우 현재 세기 (2000년대)

SELECT TO_DATE('800505', 'RRMMDD') FROM DUAL; -- 1980-05-05
SELECT TO_DATE('490505', 'RRMMDD') FROM DUAL; -- 2049-05-05
SELECT TO_DATE('800505', 'YYMMDD') FROM DUAL; -- 2080-05-05
...
--
-- 2) 추출한 생년월을 DATE 타입으로 변경 -> RR 패턴을 이용해 1900년도로 바꿈
-- TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') - 1), 'RRMMDD'))
--
-- 3) TO_CHAR를 이용해 문자열로 변환 -> 1965년 10월 08일
...
SELECT TO_NUMBER('1,000,000', '9,999,999') + 500000 합계 FROM DUAL; 
-- 1,500,000
...
-- EMPLOYEE 테이블에서
-- 급여를 가장 많이 받는 사원의 이름, 급여, 직급 코드 조회
...
-- COUNT(*) : NULL을 포함한 전체 행 개수를 리턴함.
-- COUNT([DISTINCT] 컬럼명) : 중복을 제거한 행 개수를 리턴함.

SELECT COUNT(*) FROM EMPLOYEE; -- EMPLOYEE 테이블의 행 개수 리턴 -- 23
