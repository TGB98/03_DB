 ...
 * 
 * - 데이터를 조회(SELECT)하면 조건에 맞는 행들이 조회 됨.
...
-- DUAL(DUmmy tAbLe) 테이블 : 가상 테이블(임시 조회용 테이블)
...
-- 주의사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성 되어있어야 한다.
...
-- ASC : 오름차순 , DESC : 내림차순, 기본값 : ASC
-- NULL 값이 조회 됐을 때 제일 처음에 보여줄 것인지 마지막에 보여줄 것인지 정하는 것.
...
FROM EMPLOYEE
WHERE SALARY < 3000000 OR SALARY >= 50000000;
...
-- ESCAPE 문자
-- ESCAPE 문자 뒤에 작성된 _ 는 일반 문자로 탈출 한다는 뜻.
-- #, ^
SELECT EMP_NAME, EMAIL FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 부서코드가 D1, D6, D9 인 사원의
-- 사번, 이름, 부서코드 조회
...
ORDER BY SALARY; -- ASC가 기본값(오름차순) , DESC(내림차순)

-- EMPLOYEE 테이블에서 급여가 200만원 이상인 사원의
-- 사번, 이름, 급여 조회
-- 단, 급여 내림 차순으로 조회
SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE
WHERE SALARY >= 2000000 ORDER BY SALARY DESC;

SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE
WHERE SALARY >= 2000000 ORDER BY 3 DESC; -- 컬럼 순서

-- 입사일 순서대로 이름, 입사일 조회 (별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일 FROM EMPLOYEE
ORDER BY 입사일;
...
SELECT EMP_ID, EMP_NAME, CASE WHEN SUBSTR(EMP_NO, 8, 1) = '2' THEN 'F' ELSE 'M' END 성별, SALARY, JOB_NAME, NVL(DEPT_TITLE, '없음') DEPT_TITLE
FROM EMPLOYEE
