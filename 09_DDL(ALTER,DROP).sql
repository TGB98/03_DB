...
--> NOT NULL 제약 조건은 새로운 조건을 추가하는것이 아닌
--> 컬럼 자체에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식 됨.
...
--> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입 되었음을 확인할 수 있다.

-- D10 개발1팀 추가
INSERT INTO DEPT_COPY 
VALUES ('D10','개발1팀','L1', DEFAULT, DEFAULT);
-- SQL Error [12899] [72000]:
-- ORA-12899: "KH_MKJ"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID 의 데이터 타입이 CHAR(2) 이므로 영어+숫자 2글자까지 저장 가능.
--> D10 은 CHAR(3)에 들어올 수 있다.
--> VARCHAR2(3)으로 변경해보기. (남는 Byte 메모리 반환을 위해)

-- DEPT_ID 컬럼 데이터 타입 수정.
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID VARCHAR2(3);
-- 다시 위의 INSERT 실행해서 삽입 되는지 확인


-- LNAME 의 기본값을 'KOREA' 로 수정.
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';
-- 조회해도 이미 추가되있던 데이터들은 LNAME이 '한국' 이다.
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않는다.
-- 이 후에 추가될 데이터들이 '한국' 이 아닌 'KOREA' 로 삽입 됨.
...
SELECT * FROM DEPT_COPY; -- 오류 (테이블명 변경되어서.)
...
--> 해결 방법
-- 1) TB2를 먼저 삭제하고 TB1을 삭제.
-- 2) ALTER 를 이용해서 FK 제약조건을 삭제 후 TB1 삭제 가능.
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용.
	--> CASCADE CONSTRAINTS : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제하는 옵션.

DROP TABLE TB1 CASCADE CONSTRAINTS; -- 테이블 삭제 시 FK 관계 모두 삭제.

SELECT * FROM TB1; -- 부모 테이블 삭제가 됨을 확인할 수 있음.

SELECT * FROM TB2; -- TB2 테이블만 아무 관계 없이 남게 된다.
...
SELECT * FROM TB2;
-- 롤백 안됨. 위에서 DDL 구문 중 ALTER를 사용해서 그 시점에 COMMIT이 됨.
